...
export function MonthlyView({ userId }: { userId: string }) {
  ...
  const isOrderLocked =
    (isToday(selectedDate) && now > cutoffTime) || isDatePast;
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="grid grid-cols-7 border-b border-gray-100 bg-gray-50/50">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
              <div
                key={d}
                className="py-3 text-center text-xs font-semibold text-gray-400 uppercase"
              >
                {d}
              </div>
            ))}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected &&
                      "bg-blue-50/50 ring-2 ring-inset ring-blue-500 z-10",
                  )}
                >
                  <span
                    className={cn(
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        >
          <div className="pb-4 border-b border-gray-100 mb-4">
            ...
            {isDatePast ? (
              <span className="text-xs font-semibold text-red-500 bg-red-50 px-2 py-1 rounded mt-2 inline-block">
                Past Date
              </span>
            ) : existingOrder ? (
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 mb-4">
                ...
                <div className="space-y-3 mb-4">
                  {existingOrder.items.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex gap-3 items-center bg-white p-2 rounded border border-gray-100 shadow-sm"
                    >
              ...
              <div className="mt-auto">
                {isOrderLocked ? (
                  <div className="p-3 bg-amber-50 text-amber-800 rounded-md flex items-start gap-2 text-xs border border-amber-100">
                    ...
                    <span>
                      Cancellation is unavailable. The 8:30 AM cutoff has
                      passed.
                    </span>
                  </div>
                ) : (
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button
                        variant="destructive"
                        className="w-full bg-white text-red-600 border border-red-200 hover:bg-red-50 hover:text-red-700"
                      >
                        {isCancelling ? (
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          Cancel meal for {format(selectedDate, "MMM do")}?
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                ...
                <div className="mt-6 pt-4 border-t border-gray-100 space-y-4">
                  ...
                  >
                    ...
                    {isToday(selectedDate)
                      ? "Order Now"
                      : `Pre-order for ${format(selectedDate, "MMM d")}`}
                  </Button>
                </div>
              )}
            </>
