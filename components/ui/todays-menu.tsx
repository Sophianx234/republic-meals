...
import { submitComboOrder, cancelOrder } from "@/app/actions/order"; // Import cancelOrder
    ...
    try {
      ...
      if (result.success) {
        toast.success("Order Placed", {
          description: "Kitchen has received your ticket.",
        });
  ...
  const prevImage = (e: React.MouseEvent) => {
    ...
    if (selectedItem && selectedItem.images.length > 0) {
      setCurrentImageIndex(
        (p) =>
          (p - 1 + selectedItem.images.length) % selectedItem.images.length,
      );
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4 border rounded-lg border-dashed border-gray-200 bg-gray-50/50">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          The daily selection hasn't been published yet.
        </p>
      </div>
    );
  }
      ...
      <div className="flex justify-center items-start pt-12 min-h-[60vh] p-4">
        <div className="w-full max-w-md space-y-4">
          ...
          <Alert className="bg-amber-50 border-amber-200 text-amber-900">
            ...
            <AlertDescription>
              You have an active order. You cannot place a new order unless you
              cancel this one.
            </AlertDescription>
          </Alert>
            ...
            <div className="bg-gray-50/50 p-6 border-b border-gray-100 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-mono">
                  ID: {existingOrder.id.slice(-6).toUpperCase()}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {existingOrder.items.map((item, idx) => (
                  ...
                  >
                    ...
                    <Badge
                      variant="outline"
                      className="rounded-md font-mono bg-white text-gray-600"
                    >
                      x{item.quantity}
                    </Badge>
                  </div>
                ...
                <div className="pt-4 border-t border-gray-100">
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button
                        variant="destructive"
                        className="w-full bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 hover:text-red-700 shadow-none"
                      >
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          You will need to start over if you want to eat.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
      ...
      {kitchenClosed && (
        <div className="bg-amber-50 border border-amber-200 rounded-md p-4 flex gap-3 items-center">
          ...
          <span className="text-sm font-medium text-amber-900">
            Kitchen is closed. New orders cannot be placed.
          </span>
        </div>
      )}
            ...
            >
              ...
              >
                ...
                {item.isSoldOut && (
                  <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center">
                    <span className="text-red-600 font-bold border-2 border-red-600 px-3 py-1 text-xs tracking-widest uppercase rounded-sm -rotate-12">
                      Sold Out
                    </span>
                  </div>
                )}
              ...
              <div className="p-4 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-gray-900 leading-snug pr-2 text-sm md:text-base">
                    {item.name}
                  </h3>
                  <span className="font-mono text-sm font-medium text-gray-700 bg-gray-100 px-2 py-0.5 rounded-sm">
                    ₵{item.price}
                  </span>
                </div>
          ...
          {selectedItem && (
            <div className="flex flex-col md:flex-row h-full max-h-[80vh]">
              ...
              <div className="w-full md:w-1/2 bg-gray-100 relative h-64 md:h-auto group">
                ...
                <DialogClose className="md:hidden absolute top-4 right-4 bg-black/50 text-white rounded-md p-1 z-20">
                  <X className="w-4 h-4" />
                </DialogClose>
              </div>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <Badge
                        variant="secondary"
                        className="mb-2 rounded-md bg-gray-100 text-gray-600 hover:bg-gray-100 font-normal"
                      >
          ...
          <div className="p-6 space-y-6">
            <div className="space-y-1 bg-gray-50 p-4 rounded-md border border-gray-100">
              {cart.map((item, idx) => (
                <div
                  key={idx}
                  className="flex justify-between items-center text-sm py-2 border-b border-gray-200/50 last:border-0"
                >
